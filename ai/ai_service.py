...
class AIPredictionService:
    ...
    def get_comprehensive_analysis(self, creator_data: Dict, campaign_data: Dict) -> Dict:
        ...
        # ML Predictions (now includes India XGBoost + Neural Network ensemble)
        ml_predictions = {}
        if self.ml_model:
            try:
                ml_predictions = self.ml_model.predict(features)
                print(f"  🎯 Ensemble models used: {ml_predictions.get('models_used', [])}")
            except Exception as e:
        ...
        # DL Predictions — use India NN score if available from ensemble
        dl_predictions = {}
        india_nn_score = ml_predictions.get('model_scores', {}).get('india_nn')
        if india_nn_score is not None:
            # We have a real neural network prediction from the India model
            dl_predictions = {
                'success_probability': india_nn_score / 100,
                'match_score': india_nn_score,
                'predicted_engagement': ml_predictions['estimated_engagement'],
                'model': 'india_neural_network',
                'architecture': '128-64-32-1',
            }
        elif self.dl_model and DL_AVAILABLE:
            try:
                dl_predictions = self.dl_model.predict(features)
            except Exception as e:
                print(f"DL prediction error: {e}")
                dl_predictions = {
                    'success_probability': ml_predictions['match_score'] / 120,
