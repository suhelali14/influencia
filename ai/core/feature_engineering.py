...
class FeatureEngineering:
    ...
    def compute_features(
        ...
        # Advanced features
        fv.content_velocity_score = self._compute_content_velocity(creator)
        fv.niche_authority = self._compute_niche_authority(creator, campaign)
        fv.audience_demographic_match = self._compute_audience_demographic_match(
            creator, campaign
        )
    ...
    def _compute_content_velocity(self, creator: 'Creator') -> float:
        """
        Compute content velocity score.
        
        Measures posting frequency and consistency.
        High velocity + high consistency = reliable content partner.
        """
        velocity = getattr(creator, 'content_velocity', 0.0)
        consistency = getattr(creator, 'content_consistency', 0.5)
        
        if velocity <= 0:
            return 0.3  # Unknown or inactive
        
        # Ideal posting frequency: 3-7 posts/week
        if velocity >= 3 and velocity <= 7:
            frequency_score = 1.0
        elif velocity > 7:
            # Too frequent can mean lower quality
            frequency_score = max(0.6, 1.0 - (velocity - 7) * 0.05)
        else:
            # Less than 3/week — still OK but not ideal
            frequency_score = max(0.4, velocity / 3.0)
        
        # Combine frequency with consistency (70/30 split)
        return 0.7 * frequency_score + 0.3 * consistency
    
    def _compute_niche_authority(
        self, creator: 'Creator', campaign: 'Campaign'
    ) -> float:
        """
        Compute niche authority score.
        
        Measures how deeply the creator specializes in the campaign's
        target categories. A fashion-focused creator with 50 campaigns
        in fashion scores higher than a generalist with 50 campaigns.
        """
        niche_score = getattr(creator, 'niche_authority_score', 0.5)
        
        if not creator.categories or not campaign.categories:
            return niche_score * 0.5
        
        # Check how many of the creator's categories overlap with campaign
        creator_cats = set(c.lower() for c in creator.categories)
        campaign_cats = set(c.lower() for c in campaign.categories)
        
        overlap = creator_cats & campaign_cats
        if not overlap:
            # No direct overlap — check hierarchy
            for cc in campaign_cats:
                for crc in creator_cats:
                    pair_score = self._category_pair_similarity(crc, cc)
                    if pair_score >= 0.7:
                        overlap.add(crc)  # Close enough
        
        if not overlap:
            return 0.2  # No category connection
        
        # Authority is boosted by focus (fewer categories = more specialized)
        specialization = 1.0 / max(1, len(creator_cats))
        focus_bonus = min(0.3, specialization * 0.5)
        
        # Combine niche score with overlap and focus
        overlap_ratio = len(overlap) / len(campaign_cats)
        authority = (
            0.4 * niche_score +
            0.4 * overlap_ratio +
            0.2 * focus_bonus / 0.3  # Normalize focus_bonus to 0-1
        )
        
        return np.clip(authority, 0, 1)
    
    def _compute_audience_demographic_match(
        self, creator: 'Creator', campaign: 'Campaign'
    ) -> float:
        """
        Compute audience demographic alignment score.
        
        Compares creator's audience demographics against
        campaign's target audience requirements.
        """
        demographics = getattr(creator, 'audience_demographics', {})
        
        if not demographics:
            return 0.5  # Unknown demographics, neutral
        
        scores = []
        
        # Age group match
        target_age = None
        if hasattr(campaign, 'target_age_group'):
            target_age = getattr(campaign, 'target_age_group', None)
        
        if target_age and 'age_groups' in demographics:
            age_groups = demographics['age_groups']
            # Check if target age group is the dominant one
            target_pct = age_groups.get(target_age, 0)
            max_pct = max(age_groups.values()) if age_groups else 0
            if max_pct > 0:
                scores.append(target_pct / max_pct)
            else:
                scores.append(0.5)
        
        # Gender match
        target_gender = None
        if hasattr(campaign, 'target_gender'):
            target_gender = getattr(campaign, 'target_gender', None)
        
        if target_gender and 'gender' in demographics:
            gender_data = demographics['gender']
            target_pct = gender_data.get(target_gender.lower(), 0.5)
            scores.append(target_pct)
        
        # Geographic match
        if campaign.target_locations and 'geography' in demographics:
            geo_data = demographics['geography']
            target_locs_lower = [loc.lower() for loc in campaign.target_locations]
            geo_overlap = sum(
                pct for loc, pct in geo_data.items()
                if loc.lower() in target_locs_lower
            )
            scores.append(min(1.0, geo_overlap))
        
        if not scores:
            return 0.5  # No matchable demographics
