...
class Creator:
    ...
    # Advanced profile signals
    content_velocity: float = 0.0  # Posts per week (rolling 30d)
    audience_demographics: Dict = field(default_factory=dict)  # {age_group: pct, gender: pct, geo: pct}
    brand_safety_score: float = 0.8  # 0-1, higher = safer
    avg_views: int = 0  # Average views per post
    content_consistency: float = 0.5  # 0-1, posting regularity
    niche_authority_score: float = 0.5  # 0-1, depth of expertise
...
class FeatureVector:
    ...
    content_velocity_score: float = 0.0  # Posting frequency/consistency signal
    niche_authority: float = 0.0  # Depth of expertise in matched category
    audience_demographic_match: float = 0.0  # How well audience aligns with campaign target
    ...
    def to_array(self) -> np.ndarray:
        ...
        return np.array([
            ...
            self.content_velocity_score,
            self.niche_authority,
            self.audience_demographic_match,
        ], dtype=np.float32)
