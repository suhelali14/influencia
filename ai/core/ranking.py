...
class RankingModel:
    ...
    def train(
        ...
        if use_cv and len(features) >= n_folds * 10:
            cv_metrics = self._cross_validate(features, labels, n_folds)
        ...
        metrics['val_ndcg_10'] = self._compute_ndcg(y_val, val_pred, k=10)
        metrics['val_ndcg_20'] = self._compute_ndcg(y_val, val_pred, k=20)
        
        # Add cross-validation metrics
        metrics.update(cv_metrics)
        
        self._trained_at = datetime.now().isoformat()
        self._model_version = f"2.{int(datetime.now().timestamp()) % 10000}.0"
    ...
    def _cross_validate(
        ...
        """Run stratified k-fold cross-validation for robust evaluation."""
        from sklearn.model_selection import KFold
        
        kf = KFold(n_splits=n_folds, shuffle=True, random_state=42)
        fold_metrics = {'mae': [], 'corr': [], 'ndcg_10': []}
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(features)):
            ...
            fold_metrics['mae'].append(float(np.mean(np.abs(val_pred - y_val))))
            if len(set(val_pred)) > 1 and len(set(y_val)) > 1:
                fold_metrics['corr'].append(float(np.corrcoef(val_pred, y_val)[0, 1]))
            fold_metrics['ndcg_10'].append(self._compute_ndcg(y_val, val_pred, k=10))
        
        return {
            'cv_mae_mean': float(np.mean(fold_metrics['mae'])),
            'cv_mae_std': float(np.std(fold_metrics['mae'])),
            'cv_corr_mean': float(np.mean(fold_metrics['corr'])) if fold_metrics['corr'] else 0.0,
            'cv_ndcg10_mean': float(np.mean(fold_metrics['ndcg_10'])),
        }
        ...
        try:
            ...
            if calibrator is not None:
                try:
                    ...
                    pass  # Fall through to uncalibrated
    ...
    def _rule_based_predict(self, features: np.ndarray) -> np.ndarray:
        ...
        # 30 features: 27 original + 3 new (content_velocity, niche_authority, audience_demo)
        weights = np.array([
            0.02,  # creator_followers_normalized
            0.05,  # creator_engagement_normalized
            0.07,  # creator_quality_score
            0.04,  # creator_authenticity
            0.03,  # creator_growth_rate
            0.02,  # creator_response_time_normalized
            0.05,  # creator_completion_rate
            0.04,  # creator_avg_rating
            0.05,  # creator_success_rate
            0.03,  # creator_experience_level
            0.02,  # brand_avg_rating
            0.02,  # brand_payment_reliability
            0.02,  # budget_normalized
            0.10,  # platform_match - IMPORTANT
            0.12,  # category_similarity - MOST IMPORTANT
            0.04,  # location_match
            0.03,  # language_match
            0.03,  # tier_match
            0.05,  # budget_fit
            0.03,  # followers_fit
            0.04,  # engagement_fit
            0.06,  # embedding_similarity
            0.04,  # cf_score
            0.02,  # creator_recency
            0.01,  # campaign_urgency
            0.02,  # previous_interactions
            0.04,  # previous_success
            0.03,  # content_velocity_score
            0.04,  # niche_authority
            0.03,  # audience_demographic_match
        ], dtype=np.float32)
    ...
    def compute_shap_values(
        ...
        Uses KernelSHAP when the full SHAP library isn't available
        (a lightweight approximation based on feature perturbation).
        """
        features = feature_vector.to_array()
        base_pred = self.predict_single(feature_vector)
        
        # Approximate SHAP via leave-one-out perturbation
        shap_values = {}
        for i, name in enumerate(self._feature_names):
            perturbed = features.copy()
            perturbed[i] = 0.0  # Zero out feature
            perturbed_pred = float(self.predict(perturbed.reshape(1, -1))[0])
